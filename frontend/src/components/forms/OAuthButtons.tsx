...
// Order is a design decision and lives here; which of these exist is a
// deployment fact and comes from the backend. Iterating this list rather than
// the response also means a provider the backend adds before the frontend has
// a mark for it is skipped, not rendered blank.
const PROVIDERS = [
  {
    name: "google",
...
// The divider is rendered here rather than by the caller because it introduces
// these buttons: with no provider configured there is no "or", and a caller
// holding its own divider would be left saying it above a bare email form.
export default function OAuthButtons({ divider }: { divider: string }) {
  const { t } = useTranslation();
  const { data: available } = useOAuthProviders();

  // Nothing until the list arrives, and nothing if it never does: a provider we
  // cannot confirm is one that very likely would not have worked either, and a
  // button that 404s leaves the app for a JSON error page.
  const offered = PROVIDERS.filter((p) => available?.includes(p.name));
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        {offered.map((provider) => (
          <a key={provider.name} href={oauthStartHref(provider.name)} className={base}>
            {provider.icon}
            {t(provider.labelKey)}
          </a>
        ))}
